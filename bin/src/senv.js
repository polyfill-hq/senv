import { createHmac as _createHmac, createCipheriv, createDecipheriv, pbkdf2Sync } from "crypto";
...
function encryptString(string, key, iv, name = "") {
    if (typeof key === "string") {
        key = pbkdf2Sync(key, iv, PBKDF2_ITERATION_COUNT_STRING, 32, "sha512");
    }
    if (name) {
...
function getPasswordFromEnvironment(fileName) {
    ...
    if (process.env[individualPasswordEnvVarName]) {
        return process.env[individualPasswordEnvVarName]?.trim();
    }
    ...
    if (existsSync(individualPasswordFileName)) {
        return readFileSync(individualPasswordFileName, "utf8").trim();
    }
 ...
 */
function encryptEnvFile(inputFile, outputFile = `${inputFile}.enc`, password, returnContent = false) {
    if (!password) {
    ...
    const key = pbkdf2Sync(password, salt, PBKDF2_ITERATION_COUNT_FILE, 32, "sha512");
 ...
 *                               returned as a string, otherwise returns success message.
 */
function decryptEnvFile(inputFile, outputFile = inputFile.slice(0, -4), password, returnContent = false) {
    if (!password) {
        password = getPasswordFromEnvironment(inputFile);
    }
    const inputStr = readFileSync(inputFile, "utf8");
    ...
    for (const [variableName, value] of envLines) {
        if (variableName === AUTHENTICATION_SALT
